...
def save_order():
    ...
    if user_id == house.user_id:
        return jsonify(error=RET.ROLEERR, errmsg="不嗯能够预定自己的房屋")
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(error=RET.DBERR, errmsg="检查出错，请稍后再试")
    if count > 0:
    ...
    try:
        ...
        current_app.logger.error(e)
        db.session.rollback()
        return jsonify(error=RET.DATAERR, errmsg="保存订单失败")
...
def get_user_orders():
    ...
    # 用户的身份，用户想要的查询作为房客预定别人房子的订单，还是想要作为房东查询别人预定自己房子
    ...
    return jsonify(error=RET.OK, errmsg="ok",data={"orers":orders_dict_list})
...
def accept_reject_order(order_id):
    ...
    try:
        ...
        order = Order.query.filter(Order.id == order_id, Order.stateus == "WAIT_ACCEPT").first()
        house = order.house
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(error=RET.DBERR, errmsg="无法获取订单")
    ...
    try:
        ...
        current_app.logger.error(e)
        db.session.rollback()
        return jsonify(error=RET.DBERR, errmsg="写入数据库错误")
...
def save_order_comment(order_id):
    ...
    # 将房屋详情中有订单信息的评价信息，为了让最新的评价信息展示在房屋详情中，所以删除了redis中关于本订单房屋的详情
    try:
